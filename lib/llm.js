...
// ===== Gemini Provider =====
async function callGemini(prompt, imageBase64, apiKey, config) {
...
async function callOpenAI(prompt, imageBase64, apiKey, config) {
...
async function callAzure(prompt, imageBase64, apiKey, config) {
    ...
    try {
      switch (config.provider) {
        case LLM_PROVIDERS.openai:
          rawJson = await callOpenAI(prompt, imageBase64, apiKey, config);
          break;
        case LLM_PROVIDERS.azure:
          rawJson = await callAzure(prompt, imageBase64, apiKey, config);
          break;
        default:
          rawJson = await callGemini(prompt, imageBase64, apiKey, config);
      }
